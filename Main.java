...
class Main {
  public static void main(String[] args) {
    ...
    System.out.print("DFS Search Order: ");
    for (int i = 0; i < searchOrder.size(); i++) {
        System.out.print(graph.getVertex(searchOrder.get(i)) + (i == searchOrder.size() - 1 ? "" : " -> "));
    }
    System.out.println("\n");
    ...
    for (int i = 0; i < graph.getSize(); i++) {
        if (dfs.getParent(i) != -1) {
            System.out.println("Parent: " + graph.getVertex(dfs.getParent(i)) + " | Child: " + graph.getVertex(i));
        } else {
            System.out.println("Root Node: " + graph.getVertex(i));
        }
    }
    System.out.println();

    System.out.println("Traversal Paths:");
    dfs.printPath(3);
    System.out.println();
    dfs.printPath(1);
    System.out.println();
    ...
    System.out.println("Full DFS Tree Structure:");
    dfs.printTree();
  }
}
...
class UnweightedGraph<V> implements Graph<V> {
  ...
  @Override
  public boolean addEdge(Edge e) {
    if (e.u < 0 || e.u >= getSize())
      throw new IllegalArgumentException("No such vertex: " + e.u);
    if (e.v < 0 || e.v >= getSize())
      throw new IllegalArgumentException("No such vertex: " + e.v);
